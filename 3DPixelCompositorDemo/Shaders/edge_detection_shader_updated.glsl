...
// ---- Original helpers (unchanged math) ---------------------------------------
...
vec4 GetNormal(vec2 uv, float mask){
    // Keep the tiny UV nudge exactly like original
...
void main() {
    ...
    vec2 offset        = vec2(params.tune1.w); // 0.0001 by default

    // Neighborhood (explicit K avoids .length() on some drivers)
    const int K = 4;
    vec2 uv_offsets[K];
    ...
    // Mask from normal_roughness.a, ceiled to {0,1}
    float mask = texture(normal_texture, uv_normalized + offset).a;
    mask = ceil(mask);

    // Tunables (defaults match your constants)
    float line_highlight = params.tune0.x;   // 0.1
    float line_shadow    = params.tune0.y;   // 0.55
    ...
    // ---- Depth-based Outlines (original logic) ----
    ...
    // ---- Normal-based Innerlines (original logic) ----
