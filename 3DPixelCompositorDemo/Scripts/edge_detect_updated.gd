@tool
class_name EdgeDectionCompositorUpdated
...
# ------------------------------------------------------------------------------
# Minimal artist-facing exports (defaults match your original look)
# ------------------------------------------------------------------------------
@export_range(0.0, 1.0, 0.01) var line_highlight: float = 0.10
@export_range(0.0, 1.0, 0.01) var line_shadow: float   = 0.55

# Depth edge window (smoothstep(low, high, ...))
@export_range(0.0, 1.0, 0.01) var depth_smooth_low:  float = 0.45
@export_range(0.0, 1.0, 0.01) var depth_smooth_high: float = 0.50

# Silhouette suppression (smoothstep(thresh, thresh, ...)*scale)
@export_range(0.0, 1.0, 0.01) var inv_depth_step_thresh: float = 0.90
@export_range(0.0, 20.0, 0.1) var inv_depth_scale:       float = 10.0

# Normal edge hard threshold (smoothstep(thr, thr, ...))
@export_range(0.0, 1.0, 0.01) var normal_threshold: float = 0.20

# Tiny UV nudge (kept to preserve your exact look)
...
func _init() -> void:
	...
	# Params buffer (std430):
	# 2 (raster_size) + 2 (reserved) + 16 (inv_proj) + 4 (tune0) + 4 (tune1) = 28 floats
...
func _initialize_compute() -> void:
	...
	# Compile our shader and create a compute pipeline
	var shader_file := load("res://3DPixelCompositorDemo/Shaders/edge_detection_shader.glsl")
	var shader_spirv: RDShaderSPIRV = shader_file.get_spirv()
	shader = rd.shader_create_from_spirv(shader_spirv)
	if shader.is_valid():
		pipeline = rd.compute_pipeline_create(shader)

	# Create ONE persistent sampler (nearest for pixel-crisp sampling)
...
func _render_callback(p_effect_callback_type: EffectCallbackType, p_render_data: RenderData) -> void:
	...
	# Workgroup count for 8x8 local size (integer ceil)
	...
	for view in view_count:
		# Input attachments
		var input_image: RID  = rsb.get_color_layer(view)                                # read/write image
		var input_depth: RID  = rsb.get_depth_layer(view)                                # sampled with sampler
		var input_normal: RID = rsb.get_texture("forward_clustered", "normal_roughness") # sampled with sampler

		# Defensive: skip this view if anything is invalid (resizes/hot-reloads)
		if not input_image.is_valid() or not input_depth.is_valid() or not input_normal.is_valid():
			continue

		# ---------------- Parameter buffer update (28 floats) -------------------
		...
		# 2..3: reserved (currently unused)
		params[2] = 0.0
		params[3] = 0.0

		# 4..19: inverse projection matrix (mat4 as 4 vec4 columns)
		...
		# 20..23: tune0 (line_highlight, line_shadow, depth_smooth_low, depth_smooth_high)
		params[20] = line_highlight
		params[21] = line_shadow
		params[22] = depth_smooth_low
		params[23] = depth_smooth_high

		# 24..27: tune1 (inv_depth_step_thresh, inv_depth_scale, normal_threshold, offset_scale)
		...
		u_depth.binding = 2
		u_depth.add_id(sampler_rid) # sampler first
		u_depth.add_id(input_depth) # then texture
		...
		# Use your cache helper; if it returns an invalid RID, build one directly.
